...
<jsp:setProperty name="user" property="userPassword"/> <!-- 들어갈 값이 더 많기 때문에 추가해준다. -->
...
<body>
	<% // 이 안에 자바스크립트를 사용하겠다는 의미 
		...
		int result = userDAO.join(user); // 로그인 시도( 로그인 페이지에서 넘어온 값을 입력 )
		if (result == -1){ // 로그인 성공 시,
			...
			PrintWriter script = response.getWriter();
			script.println("<script>"); 
			script.println("location.href = 'main.jsp'"); // 오류 메시지 출력과
			script.println("</script>");
		 }
